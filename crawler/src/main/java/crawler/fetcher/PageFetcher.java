...
import crawler.crawler.authentication.BasicAuthInfo;
import crawler.crawler.authentication.FormAuthInfo;
import crawler.crawler.authentication.NtAuthInfo;
import crawler.crawler.exceptions.PageBiggerThanMaxSizeException;
...
//        }
    ...
    public void reloadHttpClientByProxyOrUserAgent(CrawlConfig config, boolean rotatingProxy, boolean rotatingUserAgent) {
        synchronized (lock) {
            if(httpClient != null) {
                httpClient = null;
            }
...
//    }
    ...
    public Connection.Response fetchJsoupResponse(WebURL webUrl) throws InterruptedException, IOException, PageBiggerThanMaxSizeException {
        ...
        synchronized (lock) {
            if(config.getProxyHost() == null) {
                response = Jsoup.connect(webUrl.getURL())
                        .userAgent(config.getUserAgentString())
                        .followRedirects(true).timeout(60000).execute();
            } else {
                response = Jsoup.connect(webUrl.getURL())
